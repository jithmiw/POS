...
public class ItemDAOImpl implements ItemDAO {
    ...
    public ArrayList<Item> getAll() throws SQLException, ClassNotFoundException {
        ...
        while (rst.next()) {
            items.add(new Item(rst.getString("code"), rst.getString("description"), rst.getInt("qtyOnHand"), rst.getBigDecimal("unitPrice")));
        }
        return items;
    }
    ...
    public Item search(String itemCode) throws SQLException, ClassNotFoundException {
        ResultSet rst = SQLUtil.executeQuery("SELECT * FROM Item WHERE code=?", itemCode);
        if (rst.next()) {
